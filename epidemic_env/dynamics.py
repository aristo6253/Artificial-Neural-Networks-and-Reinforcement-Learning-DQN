...
class ModelDynamics():
    ...
    def __init__(self, source_file: str, verbose: bool = False):
        ...
        try:
            ...
            self.ACTION_CARDINALITY = len(self.NULL_ACTION)  # Is equal to 4
        ...
        # Setting the tau parameter for each road
        # i.e. the probability of infection when traveling from one city to another
        for e in self.roads:
    ...
    def reset(self):
        ...
        # Why is this 1? -> Because it's a multiplier
        ...
        # Why is this 0? -> Because it's a multiplier
    ...
    def draw_map(self,):
        ...
        nx.draw_networkx(self.map,
                         ...
                         width=[self.map.edges[e]['tau'] *
                                0.00005 for e in self.map.edges()]
                         )
    ...
    def epidemic_parameters(self, day=None) -> Tuple[Dict[str, float], Dict[str, Dict[str, float]]]:
        ...
        for c in self.cities:
            ...
            if self.verbose:
                print(
                    f"{c}: S={suceptible}, E={exposed}, I={infected}, R={recovered}, D={dead}")
    ...
    def get_action(self,):
        # Why only first city? -> Because an action applies to all cities
        _c = list(self.c_confined.keys())[0]
        # print(f'c_tot: {list(self.c_confined.keys())}')
        # print(f'_c: {_c}')
        return {'confinement': (self.c_confined[_c] != 1),          # Why is this 1?
                # Why is this 1?
    ...
    """
        ...
        _total = Observables(
            infected=[
                t.infected for t in _total_history][self.srate-1::self.srate],
            dead=[t.dead for t in _total_history][self.srate-1::self.srate],
        )
        _city = {c: Observables(
            infected=[
                t[c].infected for t in _city_history][self.srate-1::self.srate],
    ...
    def step_dyn(self):
        ...
        for c in self.cities:
            ...
            # Why not self.var_eta?
            stoch_eta = np.max([np.random.normal(self.eta, self.var_eta), 0])
