...
RewardTuple = namedtuple(
    'RewardTuple', ['reward', 'dead', 'conf', 'ann', 'vacc', 'hosp', 'isol'])
...
class Env(gym.Env):
    ...
    def __init__(self,  dyn: ModelDynamics,
                 ...
                 observation_preprocessor: Callable = lambda x, y, z: x,
                 scale: float = 100,
                 verbose: bool = False
                 ) -> None:
        """
        ...
        Modes 'binary', 'toggle, 'multi', 'factored'
        ...
        if self.verbose:
            print(f'\tEpisode length: {self.ep_len}')
            print(f'\tAction space: {self.action_space}')
            print(f'\tObservation space: {self.observation_space}')
            print(f'\tAction preprocessor: {self.action_preprocessor}')
            print(f'\tObservation preprocessor: {self.observation_preprocessor}')
    ...
    def compute_reward(self, obs: Observation) -> RewardTuple:
        ...
        def compute_death_cost():
            ...
            for city in self.dyn.cities:
                if len(obs.city[city].dead) > 1:
                    dead += DEATH_COST * \
                        (obs.city[city].dead[-1] - obs.city[city].dead[0]) / \
                        (self.dyn.total_pop)  # Not great but close enough
                else:
                    dead += DEATH_COST * \
        ...
        if self.verbose:
            print(f'\tDead: {dead}')
            print(f'\tConfinement: {conf}')
            print(f'\tAnnouncement: {ann}')
            print(f'\tVaccination: {vacc}')
            print(f'\tHospital: {hosp}')
            print(f'\tIsolation: {isol}')
            print(f'\tReward: {rew}\n')
    ...
    def get_obs(self, obs: Observation) -> torch.Tensor:
        ...
        return self.observation_preprocessor(obs, self.dyn, self.scale)     # scale added by me
